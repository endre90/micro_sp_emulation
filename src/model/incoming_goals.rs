...
pub async fn run_emultaion(
    ...
    let uq_goal_a1 = goal_string_to_sp_value(&goal_a, running::goal_runner::GoalPriority::Normal);
    let uq_goal_b1 = goal_string_to_sp_value(&goal_b, running::goal_runner::GoalPriority::Normal);
    let uq_goal_c1 = goal_string_to_sp_value(&goal_c, running::goal_runner::GoalPriority::Normal);
    let uq_goal_a2 = goal_string_to_sp_value(&goal_a, running::goal_runner::GoalPriority::Normal);
    let uq_goal_b2 = goal_string_to_sp_value(&goal_b, running::goal_runner::GoalPriority::Normal);
    let uq_goal_c2 = goal_string_to_sp_value(&goal_c, running::goal_runner::GoalPriority::Normal);

    let incoming_goals = vec![
        uq_goal_a1, uq_goal_b1, uq_goal_c1, uq_goal_a2, uq_goal_b2, uq_goal_c2,
    ]
    .to_spvalue();
    ...
    if let Some(state) = StateManager::get_full_state(&mut con).await {
        let new_state = state
            ...
            .update(&format!("{sp_id}_incoming_goals"), incoming_goals);
...
async fn test_goal_runner() -> Result<(), Box<dyn Error>> {
    ...
    let log_target = "micro_sp_emulation::test_goals_incoming";
    ...
    let (model, state) = crate::model::incoming_goals::model(&sp_id, &state);
    ...
    let emulation_handle = tokio::task::spawn(async move {
        crate::model::incoming_goals::run_emultaion(&sp_id_clone, con_local)
            .await
            .unwrap()
    });
    ...
    let con_arc_updater = con_arc.clone();
    let log_target_updater = log_target.to_string();
    let sp_id_clone = sp_id.clone();

    tokio::spawn(async move {
        tokio::time::sleep(std::time::Duration::from_secs(5)).await;
        let goal_d = "var:robot_position_estimated == d".to_string();
        let goal_e = "var:robot_position_estimated == e".to_string();
        let uq_goal_d1 =
            goal_string_to_sp_value(&goal_d, running::goal_runner::GoalPriority::Normal);
        let uq_goal_e1 =
            goal_string_to_sp_value(&goal_e, running::goal_runner::GoalPriority::Normal);
        let incoming_goals = vec![uq_goal_d1, uq_goal_e1].to_spvalue();
        let new_state = state.update(&format!("{}_incoming_goals", sp_id_clone), incoming_goals);
        let modified_state = state.get_diff_partial_state(&new_state);
        let mut connection = con_arc_updater.get_connection().await;
        StateManager::set_state(&mut connection, &modified_state).await;
        log::info!(target: &log_target_updater, "New normal priority goals injected: d, e.");

        tokio::time::sleep(std::time::Duration::from_secs(5)).await;
        let goal_f = "var:robot_position_estimated == f".to_string();
        let goal_g = "var:robot_position_estimated == g".to_string();
        let uq_goal_f1 = goal_string_to_sp_value(&goal_f, running::goal_runner::GoalPriority::High);
        let uq_goal_g1 = goal_string_to_sp_value(&goal_g, running::goal_runner::GoalPriority::High);
        let incoming_goals = vec![uq_goal_f1, uq_goal_g1].to_spvalue();
        let new_state = state.update(&format!("{}_incoming_goals", sp_id_clone), incoming_goals);
        let modified_state = state.get_diff_partial_state(&new_state);
        let mut connection = con_arc_updater.get_connection().await;
        StateManager::set_state(&mut connection, &modified_state).await;
        log::info!(target: &log_target_updater, "New high priority goals injected: f, g.");
    });
            ...
            if let SPValue::String(StringOrUnknown::String(logger_string)) = logger_sp_value {
                if let Ok(logger) = serde_json::from_str::<Vec<Vec<OperationLog>>>(&logger_string) {
                    ...
                    let expected_patterns = vec![
                        r"^\+--------------------------------------------\+$",
                        r"^\| Done -9: op_robot_move_to_a_[\w]+\s*\|$",
                        r"^\| -+\s*\|$",
                        ...
                        r"^\+--------------------------------------------\+$",
                        r"^\+--------------------------------------------\+$",
                        r"^\| Done -8: op_robot_move_to_b_[\w]+\s*\|$",
                        r"^\| -+\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Initial\s+\] Starting\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Executing\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Completing\s*\|$",
                        ...
                        r"^\+--------------------------------------------\+$",
                        r"^\+--------------------------------------------\+$",
                        r"^\| Done -7: op_robot_move_to_c_[\w]+\s*\|$",
                        r"^\| -+\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Initial\s+\] Starting\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Executing\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Completing\s*\|$",
                        ...
                        r"^\+--------------------------------------------\+$",
                        r"^\+--------------------------------------------\+$",
                        r"^\| Done -6: op_robot_move_to_a_[\w]+\s*\|$",
                        r"^\| -+\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Initial\s+\] Starting\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Executing\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Completing\s*\|$",
                        ...
                        r"^\+--------------------------------------------\+$",
                        r"^\+--------------------------------------------\+$",
                        r"^\| Done -5: op_robot_move_to_b_[\w]+\s*\|$",
                        r"^\| -+\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Initial\s+\] Starting\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Executing\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Completing\s*\|$",
                        ...
                        r"^\+--------------------------------------------\+$",
                        r"^\+--------------------------------------------\+$",
                        r"^\| Done -4: op_robot_move_to_f_[\w]+\s*\|$",
                        r"^\| -+\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Initial\s+\] Starting\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Executing\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Completing\s*\|$",
                        ...
                        r"^\+--------------------------------------------\+$",
                        r"^\+--------------------------------------------\+$",
                        r"^\| Done -3: op_robot_move_to_g_[\w]+\s*\|$",
                        r"^\| -+\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Initial\s+\] Starting\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Executing\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Completing\s*\|$",
                        ...
                        r"^\+--------------------------------------------\+$",
                        r"^\+--------------------------------------------\+$",
                        r"^\| Done -2: op_robot_move_to_c_[\w]+\s*\|$",
                        r"^\| -+\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Initial\s+\] Starting\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Executing\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Completing\s*\|$",
                        ...
                        r"^\+--------------------------------------------\+$",
                        r"^\+--------------------------------------------\+$",
                        r"^\| Done -1: op_robot_move_to_d_[\w]+\s*\|$",
                        r"^\| -+\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Initial\s+\] Starting\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Executing\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Completing\s*\|$",
                        ...
                        r"^\+--------------------------------------------\+$",
                        r"^\+--------------------------------------------\+$",
                        r"^\| Latest: op_robot_move_to_e_[\w]+\s*\|$",
                        r"^\| -+\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Initial\s+\] Starting\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Executing\s*\|$",
                        r"^\| \[\d{2}:\d{2}:\d{2}\.\d{3} \| Executing\s+\] Completing\s*\|$",

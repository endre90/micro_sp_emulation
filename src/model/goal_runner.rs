use micro_sp::{running::goal_runner::goal_predicate_to_sp_value, *};
...
pub async fn run_emultaion(
    ...
    let uq_goal_a1 =
        goal_predicate_to_sp_value(&goal_b, running::goal_runner::GoalPriority::Normal);
    let uq_goal_b1 =
        goal_predicate_to_sp_value(&goal_a, running::goal_runner::GoalPriority::Normal);
    let uq_goal_a2 =
        goal_predicate_to_sp_value(&goal_c, running::goal_runner::GoalPriority::Normal);
    let uq_goal_b2 =
        goal_predicate_to_sp_value(&goal_a, running::goal_runner::GoalPriority::Normal);
    let uq_goal_a3 =
        goal_predicate_to_sp_value(&goal_c, running::goal_runner::GoalPriority::Normal);
...
async fn test_goal_runner() -> Result<(), Box<dyn Error>> {
    ...
    let (model, state) = crate::model::goal_runner::model(&sp_id, &state);
    ...
    let emulation_handle = tokio::task::spawn(async move {
        crate::model::goal_runner::run_emultaion(&sp_id_clone, con_local)
            .await
            .unwrap()
    });

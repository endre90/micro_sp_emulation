...
pub fn model(sp_id: &str, state: &State) -> (Model, State) {
    ...
    let state = state.add(assign!(done, SPValue::Bool(BoolOrUnknown::Bool(false))), "emulator");
    ...
    let sop_struct = SOPStruct {
        id: format!("sop_move_robot").to_string(),
        sop: SOP::Alternative(vec![
            robot_move_to_pos_blocked("a", &state),
            robot_move_to_pos("b", &state),
            robot_move_to_pos("c", &state),
        ]),
    };
...
fn sop_operation_move_robot_and_gantry(sp_id: &str, state: &State) -> Operation {
    Operation::new(
        ...
        Vec::from([Transition::parse(
            &format!("complete_sop_move_robot"),
            "true",
...
fn robot_move_to_pos_blocked(pos: &str, state: &State) -> SOP {
    SOP::Operation(Box::new(Operation::new(
        &format!("robot_move_to_{}", pos),
        None,
        None,
        ...
        Vec::from([Transition::parse(
            &format!("start_robot_move_to_{pos}"),
            &format!(
                "FALSE && var:robot_request_state == initial \
                && var:robot_request_trigger == false"
            ),
            "true",
            vec![
...
#[tokio::test]
#[serial_test::serial]
async fn test_sop_alternative() -> Result<(), Box<dyn Error>> {
    use regex::Regex;
    ...
    let log_target = "micro_sp_emulation::test_sop_alternative";
    ...
    let (model, state) = crate::model::sop_alternative::model(&sp_id, &state);
    ...
    let emulation_handle = tokio::task::spawn(async move {
        crate::model::sop_alternative::run_emultaion(con_local)
            .await
            .unwrap()
    });
            ...
            if let SPValue::String(StringOrUnknown::String(logger_string)) = logger_sp_value {
                if let Ok(logger) = serde_json::from_str::<Vec<Vec<OperationLog>>>(&logger_string) {
                    ...
                    let expected_patterns = vec![
                        r"^\+--------------------------------------------\+$",
                        r"^\| Latest: op_robot_move_to_b_[\w]+\s*\|$",
                        r"^\| -+\s*\|$",

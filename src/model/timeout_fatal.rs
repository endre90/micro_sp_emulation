...
pub fn model(sp_id: &str, state: &State) -> (Model, State) {
    ...
    let state = state.add(assign!(timeout, SPValue::Bool(BoolOrUnknown::UNKNOWN)), "emulator");
...
async fn test_timeout() -> Result<(), Box<dyn Error>> {
    ...
    let (model, state) = crate::model::timeout_fatal::model(&sp_id, &state);
    ...
    let emulation_handle = tokio::task::spawn(async move {
        crate::model::timeout_fatal::run_emultaion(&sp_id_clone, con_local)
            .await
            .unwrap()
    });

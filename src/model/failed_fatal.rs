...
pub fn model(sp_id: &str, state: &State) -> (Model, State) {
    ...
    let state = state.add(
        assign!(failed, SPValue::Bool(BoolOrUnknown::Bool(false))),
        "emulator",
    );
    ...
    operations.push(Operation::new(
        ...
        false, // We can add bypass transitions, but usually not necessary to bypass
...
pub async fn run_emultaion(
    ...
    let goal = "var:gantry_locked_estimated == false".to_string();
...
#[tokio::test]
#[serial_test::serial]
async fn test_failed_fatal() -> Result<(), Box<dyn Error>> {
    use regex::Regex;
    ...
    let log_target = "micro_sp_emulation::test_failed_fatal";
    ...
    let (model, state) = crate::model::failed_fatal::model(&sp_id, &state);
    ...
    let emulation_handle = tokio::task::spawn(async move {
        crate::model::failed_fatal::run_emultaion(&sp_id_clone, con_local)
            .await
            .unwrap()
    });

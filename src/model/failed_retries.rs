...
pub fn model(sp_id: &str, state: &State) -> (Model, State) {
    ...
    let state = state.add(assign!(failed, SPValue::Bool(BoolOrUnknown::Bool(false))), "emulator");

    let bypassed = bv!(&&format!("bypassed"));
    let state = state.add(assign!(bypassed, SPValue::Bool(BoolOrUnknown::Bool(false))), "emulator");

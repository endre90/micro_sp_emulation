...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let (model, state) = model::timeout_disabled::model(&sp_id, &state);
    ...
    let sp_id_clone = sp_id.clone();
    tokio::task::spawn(async move { model::timeout_disabled::run_emultaion(&sp_id_clone, con_local).await.unwrap() });

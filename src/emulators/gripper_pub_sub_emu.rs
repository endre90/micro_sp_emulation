...
pub async fn subscriber_callback(
    ...
    mut subscriber: impl Stream<Item = GripperOutgoing> + Unpin,
    node_id: &str,
        ...
        match subscriber.next().await {
            Some(message) => {
                ...
                let emulator_state_local =
                    emulator_state_local.update("gripper_ref", message.gripper_ref.to_spvalue());
...
pub async fn publisher_callback(
    ...
    loop {
        let emulator_state_local = emulator_state.lock().unwrap().clone();
        let msg = GripperIncoming {
            gripper_act: match emulator_state_local.get_value("gripper_act") {

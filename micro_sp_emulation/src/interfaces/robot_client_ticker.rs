...
pub async fn robot_client_ticker(
    ...
    loop {
        if let Some(state) = redis_get_state_for_keys(&mut con, &keys).await {
            ...
            let robot_speed_command =
                state.get_float_or_default_to_zero(target, "robot_speed_command");
            ...
            let emulated_failure_cause_sp_value =
                state.get_array_or_default_to_empty(target, "robot_emulated_failure_cause");
